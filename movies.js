...
const movieListEl = document.querySelector('movie__results');
    ...
    <figure class="movie__poster">
        <img src="${movie.Search.Poster}" alt="" class="poster">
    </figure>
       <div class="movie__description">
        <h4 class="movie__title">${movie.Search.Title}</h4>
        <p class="movie__genre">${movie.Search.Genre}</p>
        <p class="movie__runtime">${movie.Search.Runtime}</p>
    </div>
    </div>
     `
     }